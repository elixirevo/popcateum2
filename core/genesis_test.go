...
func TestInvalidCliqueConfig(t *testing.T) {
	block := DefaultHappycatGenesisBlock()
...
func TestSetupGenesis(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "custom block in DB, genesis == happycat",
			fn: func(db ethdb.Database) (*params.ChainConfig, common.Hash, error) {
				customg.MustCommit(db)
				return SetupGenesisBlock(db, trie.NewDatabase(db), DefaultHappycatGenesisBlock())
			},
			wantErr:    &GenesisMismatchError{Stored: customghash, New: params.HappycatGenesisHash},
			wantHash:   params.HappycatGenesisHash,
			wantConfig: params.HappycatChainConfig,
		},
		{
...
func TestGenesisHashes(t *testing.T) {
	for i, c := range []struct {
		...
		{DefaultHappycatGenesisBlock(), params.HappycatGenesisHash},
