...
var (
	...
	NetworkIdFlag = &cli.Uint64Flag{
		Name:     "networkid",
		Usage:    "Explicitly set network id (integer)(For testnets: use --rinkeby, --happycat, --sepolia instead)",
...
func MakeDataDir(ctx *cli.Context) string {
	if path := ctx.String(DataDirFlag.Name); path != "" {
		...
		if ctx.Bool(HappycatFlag.Name) {
			return filepath.Join(path, "happycat")
		}
...
func setBootstrapNodes(ctx *cli.Context, cfg *p2p.Config) {
	...
	case ctx.Bool(HappycatFlag.Name):
		urls = params.HappycatBootnodes
	}
...
func SetDataDir(ctx *cli.Context, cfg *node.Config) {
	...
	case ctx.Bool(HappycatFlag.Name) && cfg.DataDir == node.DefaultDataDir():
		cfg.DataDir = filepath.Join(node.DefaultDataDir(), "happycat")
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *ethconfig.Config) {
	...
	CheckExclusive(ctx, MainnetFlag, DeveloperFlag, RinkebyFlag, HappycatFlag, SepoliaFlag)
	...
	case ctx.Bool(HappycatFlag.Name):
		if !ctx.IsSet(NetworkIdFlag.Name) {
			cfg.NetworkId = 112255
		}
		cfg.Genesis = core.DefaultHappycatGenesisBlock()
		SetDNSDiscoveryDefaults(cfg, params.HappycatGenesisHash)
	case ctx.Bool(DeveloperFlag.Name):
		if !ctx.IsSet(NetworkIdFlag.Name) {
			cfg.NetworkId = 1337
		}
...
func MakeGenesis(ctx *cli.Context) *core.Genesis {
	...
	case ctx.Bool(HappycatFlag.Name):
		genesis = core.DefaultHappycatGenesisBlock()
	case ctx.Bool(DeveloperFlag.Name):
